...
export async function getTransactions(token: string): Promise<Array<Income | Expense>> { const [incomes, expenses] = await Promise.all([request<Income[]>('/api/v1/finance/incomes', {}, token), request<Expense[]>('/api/v1/finance/expenses', {}, token)]); return [...incomes, ...expenses].sort((a,b)=>b.transactionDate.localeCompare(a.transactionDate)); }
